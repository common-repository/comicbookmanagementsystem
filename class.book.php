	...
	class cbms_Book
	{
		...
		public function storeFormValues( $params ) {
			...
			if( isset( $params['releasedate'] ) ) 
			  {
			    $releaseDate = explode ( '-', $params['releasedate'] );
		 
			    if( count( $releaseDate ) == 3 ) 
				  {
					list ( $year, $month, $date ) = $releaseDate;
					$this->releasedate = ( $year . "-" . $month . "-" . $date );
				  }
			  }		
		}
			...
			if( count( $books ) > 0 )
			  {
				 foreach( $books as $book )
				 {
				   ...
				   $upcomingbook = new cbms_Book($array);
				   $list[] = $upcomingbook;
				 }
				return($list); 
			  }						
		...
		public function deleteImages() {
			foreach( glob( trailingslashit( WP_CONTENT_DIR ) . 'cbms_Covers/'  . $this->id . $this->img) as $filename )
			{
			   if( !unlink( $filename ) ) 
				 {
		...
		public function getImagePath() {
			return( $this->id && $this->img ) ? (trailingslashit( WP_CONTENT_DIR ) . 'cbms_Covers/'  . $this->id . $this->img ) : false;
		}
